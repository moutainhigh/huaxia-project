...
public class WorkflowCallbackThreadTierNode22 implements Runnable {
	
	private static final Logger logger = LoggerFactory.getLogger(WorkflowCallbackThreadTierNode22.class);
	...
	public WorkflowCallbackThreadTierNode22(){
	...
	public void run() {
		...
		while(true){
			try{
				...
				List<TaskCallWorkflow> workflowTaskList = workflowCallbackService.queryWorkflowTaskList(querySize, 22);
					...
					for (TaskCallWorkflow workflowTask : workflowTaskList){
						try{
					        ...
					        if ("1".equals(responseCode)) {
					            ...
					            logger.info("第22梯队线程回调工作流成功[{}]!", workflowTask.getAppId(), responseCode);
