 ...
 *
 */
public class WST001102TaskWorker implements Runnable {
	// 日志
	private final static Logger logger = LoggerFactory.getLogger(WST001102TaskWorker.class);
	// 引入参数
	...
	public WST001102TaskWorker() {
		...
		this.querySize = Integer.valueOf(appConfig.getValue("unicom.address.work.query_size"));
		this.taskType = appConfig.getValue("unicom.address.work.task_type");
		this.uniaddinforUrl = appConfig.getValue("unicom.address.work.plaze.webservice.url");
		this.readOuttime = Integer.valueOf(appConfig.getValue("unicom.address.work.webservice.read_timeout"));
			...
			try {
				...
				if (taskStatusList != null && taskStatusList.size() > 0) {
					if (logger.isDebugEnabled()) {
						logger.debug("[客户端 & 联通地址类信息(工作地址)] 查询到联通地址类信息(工作地址)任务条数：{}", taskStatusList.size());
					}
					// 遍历集合,发起查询
					for (Map<String, String> task : taskStatusList) {
						// 获取appId
						String appId = task.get("APP_ID");
						int countAddress = unicomAddressService.countByAppId(appId,"360015");
						...
						try {
							...
							if (logger.isErrorEnabled()) {
								logger.error("[客户端 & 联通地址类信息(工作地址)] 申请件[{}]调用第三方查询平台服务异常:{}", appId, e);
							}
							// 保存历史表 taskType=1
							taskCallStatusService.saveTaskStatusHistory(appId, taskType);
...
//						logger.info("联通工作地址接收三方平台的返回报文>>"+jsonResponse.toString());
						...
						// 三方查询平台数量上限999994,不在查询时间范围内999993
							...
							// 保存产品ID
							unicomAddress.setApiKey("360015");
							...
							if (code != null && !"".equals(code) && "00".equals(code)) {
								try {
									...
									// 入库异常,将状态改为12入库异常
									if (logger.isErrorEnabled()) {
										logger.error("[客户端 & 联通地址类信息(工作地址)] 报文数据持久化失败   " + appId + " Exception:{}", e);
									}
									// 保存历史表 taskType=1
									taskCallStatusService.saveTaskStatusHistory(appId, taskType);
									// 修改状态 1改12,入库异常
					...
					if (logger.isDebugEnabled()) {
						logger.debug("[客户端 & 联通地址类信息(工作地址)] 未查询到联通地址类信息(工作地址)任务条数!");
					}
				}

			} catch (Exception e) {
				if (logger.isErrorEnabled()) {
					logger.error("[客户端 & 联通地址类信息(工作地址)] 线程异常中断 重启  Exception:{}", e);
				}
			}
		}
	}
