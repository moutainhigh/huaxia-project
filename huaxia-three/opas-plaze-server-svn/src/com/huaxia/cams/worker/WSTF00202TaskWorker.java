 ...
 * @author User
 */
public class WSTF00202TaskWorker implements Runnable{
	//insert into task_call_worker values('F00202','外国人永久居留证信息核查服务附卡','com.huaxia.cams.worker.WSTF00202TaskWorker','');
	private static final Logger logger = LoggerFactory.getLogger(WSTF00202TaskWorker.class);
	...
	public WSTF00202TaskWorker(){
		//获取配置参数
		taskType = appConfig.getValue("police.supkar.foreigner.task_type");
			...
			try {
				...
				if(taskStatusList != null && taskStatusList.size()>0){
					...
					for(TaskCallStatus task :taskStatusList){
						...
						//从OPAS_BIZ_INP_APPL_C2表查询出生日期和证件有效期
						Map<String,String> map = policeForeignerService.getSupKarBirthAndIDTE(task.getAppId());
						if(map == null || map.size() == 0){
							paramFlag = true;
							if(logger.isInfoEnabled()){
								logger.info("【外国人永久居留证】 请求OPAS_BIZ_INP_APPL_C2的出生日期和证件有效期为空，流水号为:{}",appId);
							}
						}else{
							Set keys = map.keySet();
						...
						try {
							responseParse( String.valueOf( result[0] ), appId);
						} catch (Exception e) {
							if(logger.isErrorEnabled()){
	...
	private void responseParse(String str, String appId) {
		...
		if (message != null && !"".equals(message) && "000000".equals(code)) {
			...
			try {
				if(nciicForeignerInfo != null){
					...
					nciicForeignerInfo.setCardFlag("1");
	...
	public static void main(String[] args) {
		...
		Thread thread = new Thread(new WSTF00202TaskWorker());
		thread.start();
	}
}
