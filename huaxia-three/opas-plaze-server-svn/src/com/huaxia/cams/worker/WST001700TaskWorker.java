...
import com.huaxia.cams.modules.yilianzhong.util.YlzResponseParseHelper;
...
/**
 * 区域数据-厦门-易联众  接口调用给处理类
 * @author User
 *
 */
public class WST001700TaskWorker implements Runnable{

	private final static Logger logger = LoggerFactory.getLogger(WST001700TaskWorker.class);
...

	{
		...
		taskType = appConfig.getValue("area.yilianzhong.task_type");
		requestUrl = appConfig.getValue("area.yilianzhong.plaze.webservice.url");
		querySize=Integer.parseInt(appConfig.getValue("area.yilianzhong.query_size"));
		recordService=(RecordService)SpringContextUtil.getBean("recordService");
		ylzRepDataService=(YlzRepDataService)SpringContextUtil.getBean("ylzRepDataService");
	}
		...
		while (true) {
			try {
				...
				if (taskStatusList != null && taskStatusList.size() > 0){
					logger.debug("[客户端 & 区域数据-厦门-易联众] 查询到个人信用任务条数：{}", taskStatusList.size());
					/** 遍历集合,逐条发起查询请求 */
					for (TaskCallStatus task : taskStatusList){
						...
						int countRecord = recordService.selectYlzCountByAppId(appId);
						...
						if(StringUtils.isBlank(name)){
							...
							if (logger.isDebugEnabled()) {
								logger.debug("[客户端 & 区域数据-厦门-易联众] 请求的姓名为空，流水号为：{}", appId);
							}
						}
						if(StringUtils.isBlank(certNo)){
							paramErrorFlag = true;
							if (logger.isDebugEnabled()) {
								logger.debug("[客户端 & 区域数据-厦门-易联众] 请求的证件号为空，流水号为：{}", appId);
							}
						}
						...
						if(StringUtils.isNotBlank(result)){
							YlzRepData ylzRepData=YlzResponseParseHelper.getQueryModel1Response(result, appId);
							if (ylzRepData == null) {
								logger.error("[区域数据-厦门-易联众] 解析三方平台返回报文异常");
								taskCallStatusService.saveTaskStatusHistory(appId, taskType);
							...
							ylzRepData.setCrtUser("PLAZE");
							//添加证件号码
							ylzRepData.setCertNo(certNo);
							// 保存解析后的返回报文
							try{
				...
				if (logger.isErrorEnabled()) {
					logger.error("[客户端 & 厦门易联众] 线程异常中断 重启  Exception:{}", e);
				}
			}
	...
	public static String getRequestJson(String trnId,String certNo,String name){
		...
		responseHeadObjectDmzReq.put("TRN_CODE", "WST001700");
	...
	public static void main(String[] args) {
		...
		Thread thead  =new Thread(new WST001700TaskWorker());
		thead.start();
	}
