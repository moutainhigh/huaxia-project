...
import com.huaxia.opas.interfaces.out.ReceiveSingleBankSfpt;
...
public class ReceiveSingleBankSfptImpl implements ReceiveSingleBankSfpt {

	private final static Logger logger = LoggerFactory.getLogger(ReceiveSingleBankSfptImpl.class);
	...
	public String getBankBackMessage(String json) {
		if (json == null || "".equals(json)) {
			if (logger.isWarnEnabled()) {
				logger.warn("[客户端 & BANK] 接收三方平台响应信息为空!!!");
			}
			return "failure";
		}else{
			if (logger.isInfoEnabled()) {
				 logger.info("bankResponseSuccess:{}","BANK SF back success!!");
			 }
		}
	    ...
	    String  sourceId = "";//关联id
	    String responseBody = "" ;//响应体
	    String responseCode = "" ;//响应代码
		if( jsonObject.containsKey("RESPONSE") && !"".equals(jsonObject.getString("RESPONSE")) ){
			JSONObject jsonResponse =JSONObject.fromObject(jsonObject.getString("RESPONSE"));
			
			if( jsonResponse.containsKey("HEAD") && !"".equals(jsonResponse.getString("HEAD")) ){
				JSONObject jsonResponseHead =JSONObject.fromObject(jsonResponse.getString("HEAD"));
				
				/*if( jsonResponseHead.containsKey("REQUEST_SYSTEM") && !"".equals(jsonResponseHead.getString("REQUEST_SYSTEM")) ){
				String 	REQUEST_SYSTEM = jsonResponseHead.getString("REQUEST_SYSTEM");
				}*/
				if( jsonResponseHead.containsKey("TRN_ID") && !"".equals(jsonResponseHead.getString("TRN_ID")) ){
					sourceId = jsonResponseHead.getString("TRN_ID");
				}
			}
			
			if( jsonResponse.containsKey("BODY") && !"".equals(jsonResponse.getString("BODY")) ){
				JSONObject jsonRequestBody =JSONObject.fromObject(jsonResponse.getString("BODY"));
				responseBody = jsonRequestBody.getString("RESPONSE_BODY");
				responseCode = jsonRequestBody.getString("RESPONSE_CODE");
				if("AAA001".equals(responseBody)){
					taskCallStatusService.saveTaskCallStatusInteractHis(sourceId, BANK_TASK_TYPE);
					taskCallStatusService.updateSingleTaskInteract(sourceId, TaskStatusUtil.SUCCESS, BANK_TASK_TYPE, 
		...
		/*		
   			000000  	处理成功
   			000002  	关键元素项校验不通过      	  3   请求参数异常    
   			999997		征信数据源服务请求超时           7  对方服务响应异常
		 */
       String resTaskStatus = TaskStatusUtil.SUCCESS ;//返回的任务状态默认是 成功 2 
       boolean resCodeFlag = false ; // 报文返回码标识 默认false
		if( "000002".equals(responseCode) ){//请求参数异常    
			resTaskStatus = TaskStatusUtil.PARAM_ERROE;
			resCodeFlag = true;
		}
		if( "999997".equals(responseCode) ){//对方服务响应异常
			resTaskStatus = TaskStatusUtil.SERVE_RESPOSE_ERROE;
			resCodeFlag = true;
		}
		if( resCodeFlag ){
			//保存历史记录
			...
			taskCallStatusService.updateSingleTaskInteract(sourceId, resTaskStatus, BANK_TASK_TYPE, 
					null, TaskStatusUtil.CURR_USER , null, null);
	...
	/*	Calendar calendar = Calendar.getInstance();
		...
		try {
			documentBody = DocumentHelper.parseText(responseBody);
		} catch (DocumentException e) {
			if (logger.isInfoEnabled()) {
				 logger.info("bankDocumentBodyException:{}",e);
			 }
		}
	String APP_ID = taskCallStatusService.findAppIdTaskStatusInteract(sourceId, 
	...
	if( APP_ID != null && !"".equals(APP_ID) ){
			...
			CommonUtil.downLoadMessageContent(responseBody, getXmlSavePath(), APP_ID, null, ".xml","人行信息");
			Bank bank = null;
		    ...
		    try{
		    	taskCallStatusService.updateSingleTaskInteract(sourceId, taskStatus, BANK_TASK_TYPE, 
						null, TaskStatusUtil.CURR_USER , null, failureAddNum);
