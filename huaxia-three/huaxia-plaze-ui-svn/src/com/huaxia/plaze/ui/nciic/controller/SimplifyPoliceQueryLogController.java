 ...
 */
@Controller
@RequestMapping(value = "nciic/police/log")
public class SimplifyPoliceQueryLogController {

	private final static Logger logger = LoggerFactory.getLogger(SimplifyPoliceQueryLogController.class);
	...
	public String findLogListByPage(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("[简项公安查询日志监控] 查询参数[{}]", new Object[] { page.getDataMap() });
		}

		int totalCount = simplifyPoliceService.queryLogListCountByPage(page);
		List<SimplifyPoliceQueryLog> logList = simplifyPoliceService.queryLogListByPage(page);
		StringBuilder builder = new StringBuilder();
		if (logList != null && logList.size() > 0) {
			for (SimplifyPoliceQueryLog pql : logList) {
				// 证件号码第4-14位用*号代替
