package com.huaxia.plaze.ui.baoxin.controller;
...
/** 宝信汽车风控类数据查询（配置）*/
@Controller
@RequestMapping(value = "baoxin/automobile/configurate")
public class BaoxinConfigurateController {

	@Resource
	private QueryConfigurateService queryConfigurateService;

	@Resource
	private BaoxinConfigurateService baoxinConfigurateService;

	@RequestMapping("query/index")
	public String index(ModelMap modelMap) throws JsonProcessingException {
		return "baoxin/baoxin_query_setting";
	}
	...
	public String queryCount(String startTime, String endTime) {
		...
		List<Map<String, Integer>> queryCountList = baoxinConfigurateService.queryCountByParams(args);
	...
	public String saveQuerySetting(HttpServletRequest request, @RequestBody String jsonString)
		...
		response.put("message", "宝信汽车风控类查询设置失败!");
		int result = queryConfigurateService.mergeSetting(args);
		if (result > 0) {
			response.put("result", true);
			response.put("message", "宝信汽车风控类查询设置成功!");
		}
