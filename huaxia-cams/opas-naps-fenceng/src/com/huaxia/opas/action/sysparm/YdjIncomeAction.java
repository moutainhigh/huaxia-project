 ...
 *
 */
public class YdjIncomeAction implements Action{


	@Resource(name = "ydjIncomeService")
	private YdjIncomeService ydjIncomeService;

	/**
	 * 查询
	 */
	public void queryIncome(Context ctx) {
		Gson gson = new Gson();
		...
		int count = ydjIncomeService.queryIncomeCount(income);
		if (count > 0) {
			list = ydjIncomeService.queryIncomeList(income, curNum, pageNum);
		}
	...
	public void saveIncome(Context ctx) throws CoreException {
		...
		try {
			ydjIncomeService.insertIncome(income);
	...
	public void updateIncome(Context ctx) throws CoreException {
		...
		Income income = json.fromJson(json.toJson(ctx.getDataMap()), Income.class);
		try {
			ydjIncomeService.updateIncome(income);
		} catch (CoreException e) {
			ctx.setData("exMsg", e.getMessage());
			throw e;
		}
	...
	public void deleteIncome(Context ctx) throws CoreException {	
		try {
			...
			ydjIncomeService.deleteIncome(income);
			ctx.setData("isSuccess", true);
		} catch (CoreException e) {
	...
	public void setIncomeStatusOK(Context ctx) throws CoreException{
		try {
			...
			ydjIncomeService.setIncomeStatusOK(income);
			ctx.setData("isSuccess",true);
		}catch(CoreException e){
			ctx.setData("exMsg",e.getMessage());
			ctx.setData("isSuccess",false);
	...
	public void setIncomeStatusNO(Context ctx) throws CoreException{
		try {
			Income income = getIncome(ctx,"ids","Status");
			ydjIncomeService.setIncomeStatusNO(income);
			ctx.setData("isSuccess",true);
		}catch(CoreException e){
			ctx.setData("exMsg",e.getMessage());
			ctx.setData("isSuccess",false);
