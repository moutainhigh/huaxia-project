 ...
 *
 */
public class Step3_Field_Yc_Gjj_Info implements RiskFieldMachService {
	...
	public List<RiskCheckResult> macheField(String appId, RiskCheck_Apply_C1 apply_Info1,
		...
		if ("63".equals(appId.substring(8,10))) {
		
			RiskCheckResult check_STATUS = new RiskCheckResult("STATUS", "YC_INFO",
					CacheConsts.RISK_TYPE_THIRDPARTY);
			check_STATUS.setApplyType(appType);
			RiskCheckResult check_COMPANY = new RiskCheckResult("COMPANY", "YC_INFO",
					CacheConsts.RISK_TYPE_THIRDPARTY);
			check_COMPANY.setApplyType(appType);
			listriskcheck.add(check_STATUS);
			listriskcheck.add(check_COMPANY);
			String company = riskCheckService.Query_YC_INFO(appId);
			...
			if(!"".equals(trimToEmpty(company))){
				if (lueneService.getMachResult(companyName, company, "")) {
					...
					Float month = riskCheckService.Query_YC_INFODate(param);
					if (month != null) {
						if (month > 0 &&month <= 6) {
