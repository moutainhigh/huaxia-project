 ...
 *
 */
public class Step2_Field_Opas_Biz_IdNumber_InDate implements RiskFieldMachService {
	...
	public List<RiskCheckResult> macheField(String appId, RiskCheck_Apply_C1 apply_Info1,
		...
		if (!"2".equals(appType)) {
			RiskCheckResult check_IdIndate = new RiskCheckResult("C5_IDTE1", "OPAS_BIZ_INP_APPL_C1",
					CacheConsts.RISK_TYPE_TEAM);
			RiskCheckResult check_Blaze = new RiskCheckResult("C5_IDTE2", "OPAS_BIZ_INP_APPL_C1",
					CacheConsts.RISK_TYPE_TEAM);
			...
			if (idNumberInDate != null) {
				...
				if("01".equals(StringUtil.trimToEmpty(apply_Info1.getC1Idtype()))){ //证件为身份证
					if ("2999-12-31".equals(formatInDate)) {
						/*if("01".equals(StringUtil.trimToEmpty(apply_Info1.getC1Idtype()))){ //证件为身份证
							years = riskCheckService.queryAgeByDateOfIssue(appId);
						}else{					
							years = riskCheckService.queryYearBetweenIssuingDate(appId);
						}*/	
				...
				if (!flag) {
					...
					map.put("appId", appId);
					//证件有效期=c1表证件有效期匹配-“20”+appid前六位 得到天数
