...
public class Bootstrap {
	...
	protected ApplicationContext initApplicationContext() throws BeansException {
		if (log.isInfoEnabled()) {
			log.info("tcp dispatcher will try to create custom ApplicationContext context of class '" + contextClass.getName() + "'");
		}
		
		ClassPathXmlApplicationContext xmltcpapplicationcontext = (ClassPathXmlApplicationContext) BeanUtils.instantiateClass(contextClass);
		xmltcpapplicationcontext.setParent(null);
		
		// 设置上下文配置
		if (contextConfigLocation != null) {
			xmltcpapplicationcontext.setConfigLocations(StringUtils.tokenizeToStringArray(contextConfigLocation, " ,;", true, true));
		}
		
		// 刷新上下文
		xmltcpapplicationcontext.refresh();
		return xmltcpapplicationcontext;
	}
