...
import com.huaxia.opas.service.rule.FicoRuleService;

public class FicoRuleServiceImpl implements FicoRuleService{

	private static final Logger log = Logger.getLogger(FicoRuleServiceImpl.class);
	...
	public Map<String, Object> invokeRule(String appId) throws Exception {
		...
		log.info("fico查询平台规则引擎服务接口开始!!!");

		// 判断为哪个节点的规则
		Opasbizinpapplc1 on1 = plazeRuleModelBuildService.queryOpasbizinpapplc1(appId);
		FicoGradeInfo fgi = plazeRuleModelBuildService.queryFicoMsg(on1);
		LoansInfo lsi = plazeRuleModelBuildService.queryLoansMsg(on1);
		// 主卡申请件信息
		// 构造规则集执行会话
		...
		inParams.put("FicoGradeInfo", fgi);
		inParams.put("LoansInfo", lsi);
		inParams.put("SingleRuleOut", new SingleRuleOut());

		// 设置请求对象参数
		TopRulesRequest request = new TopRulesRequest();
		request.setProjectName("project05");
		...
		request.setBusinessNo("fico业务查询平台");
		...
		try {
			...
			if (outParams != null) {
				...
				outPut.put("fico_rule_out", sroout);
				log.info("规则输出信息:" + sroout.toString());
//				System.out.println("规则输出信息:" + sroout.toString());
//				System.out.println("规则输出信息:" + sroout.get(0) instanceof String);
			}
		} catch (Exception e) {
