(function($) {
	...
	$.fn.extend({
		 ...
		 * 样例：
		 * <ul>
		 *   <li>加载结果集：{"json":[{"key":0,"value":"内容0"}]}</dd>
		 *   <li>加载结果集并选中指定项：{"json":[{"key":0,"value":"内容0"},{{"key":1,"value":"内容1"],"selected":false,"key":"1","ignore":false}</dd>
		 * <ul>
		...
		fnLoadComponent : function(settings) {
			...
			var object = $.parseJSON(settings);
			var $this = $(this), key = object.key, selected = object.selected, ignore = object.ignore;
			if ('select-one' == $this[0].type) {
				$this.empty();
				var options = '';
				if (!ignore) {
					options += '<option value="">请选择</options>';
				}
				$.each(object.json, function(i, o) {
